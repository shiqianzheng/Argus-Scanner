...
| 参数分类 | 命令参数 | 描述 | 示例 |
| :--- | :--- | :--- | :--- |
| **基础** | `target` | 扫描目标 (文件或目录) | `python main.py ./src` |
| | `-f, --format` | 报告格式: `html`, `json`, `text` | `-f json` |
| | `-o, --output` | 报告输出目录 (默认: `./reports`) | `-o ./audit_logs` |
| | `--config` | 指定自定义配置文件路径 | `--config ./my_config.yaml` |
| | `-v, --verbose` | 显示详细调试日志 | `-v` |
| **分析模式** | `--static-only` | 仅进行静态语义分析 | `--static-only` |
| | `--dynamic-only`| 仅在 Docker 沙箱中进行动态分析 | `--dynamic-only` |
| | `--language` | 指定语言: `python`, `java`, `go`, `c`, `cpp`, `auto` | `--language go` |
| **沙箱控制** | `--manual-setup` | **手动配置模式** (跳过自动依赖安装) | `--manual-setup` |
| | `--exec-cmd` | **自定义执行命令** (覆盖自动探测的入口) | `--exec-cmd "go run main.go"` |
| | `--docker-image` | **指定镜像** (覆盖默认语言镜像) | `--docker-image python:3.9` |
| | `--clean` | **清理环境** (删除该项目对应的沙箱容器) | `--clean` |

### 3. 核心参数详解

#### 🔧 手动配置模式 (`--manual-setup`)
当自动依赖安装失败，或需要复杂的环境配置（如安装系统级库、配置数据库连接）时使用。
- **行为**: 程序启动容器后会暂停，并提示您手动进入容器（`docker exec -it <container_id> /bin/bash`）。您可以在容器内修复环境，完成后回到扫描器按回车继续。

#### 🚀 自定义执行命令 (`--exec-cmd`)
默认情况下，Argus-Scanner 会尝试智能识别项目入口（如 `manage.py`, `pom.xml`）。对于非标准项目，您可以通过此参数精确控制运行逻辑。
- **示例**: `python main.py ./app --exec-cmd "cd sub_module && mvn spring-boot:run"`

#### 🐳 指定 Docker 镜像 (`--docker-image`)
根据项目需求指定特定的运行环境。支持本地镜像或 Docker Hub 镜像。
- **示例**: `python main.py ./old_app --docker-image python:3.6-slim`

#### 🧹 清理沙箱 (`--clean`)
Argus-Scanner 默认会**复用**容器以加速二次扫描（持久化容器命名为 `argus_sandbox_<hash>`）。如果您需要一个纯净的环境，或遇到环境污染问题，请使用此参数强制删除旧容器并重新创建。
